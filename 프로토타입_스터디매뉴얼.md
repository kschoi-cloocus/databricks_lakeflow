...
- [Part 5. 심화: DQX 데이터 품질 프레임워크 (SDP 보완)](#part-5-심화-dqx-데이터-품질-프레임워크-sdp-보완)
- [Part 6. 겪은 오류에서 배우기 (트러블슈팅)](#part-6-겪은-오류에서-배우기-트러블슈팅)
- [Part 7. 스스로 점검 & 확장 과제](#part-7-스스로-점검--확장-과제)
- [부록. 빠른 참조](#부록-빠른-참조)
...
#### 용어 정리 — 역할(일반명사) ↔ 실체(우리 프로토타입)

메시징 용어(생산자·브로커·소비자)를 실제 컴포넌트에 붙이면 헷갈리지 않습니다.

| 일반명사 | 뜻 | **우리 프로토타입의 실체** |
|---|---|---|
| **생산자(Producer)** | 데이터를 만들어 **보내는** 쪽 | 단말기 SN-0001~0020 (흉내: `02_inject_eventhub.py`) |
| **브로커(Broker) = 중개자** | 메시지를 받아 **맡아두고 전달**하는 중간 시스템 | **Event Hub** (Azure 서비스 자체) |
| **소비자(Consumer)** | 데이터를 **읽어가는** 쪽 | **Databricks SDP** (`bronze_telemetry`가 kafka로 읽음) |

```
 생산자                브로커 (Event Hub)                    소비자
 ┌──────────┐        ┌───────────────────────────┐      ┌──────────────┐
 │ 단말기    │ 이벤트 │ namespace: ehns-iotproto   │ 읽기 │ Databricks   │
 │ SN-0001~ │ ─────▶ │  └ hub: device-telemetry   │ ───▶ │ SDP          │
 │  0020    │        │      └ partition 0, 1       │      │ bronze_...   │
 └──────────┘        └───────────────────────────┘      └──────────────┘
     (보낸다)              (며칠 맡아둔다)                    (읽어 Delta에 영구저장)
```
...
| 용어 | 뜻 | 우리 것 |
|---|---|---|
| Namespace | hub들을 담는 그릇(인프라 단위) | `ehns-iotproto-kc22` |
| Event Hub(= hub) | 실제 스트림 한 줄기 (= Kafka topic) | `device-telemetry` |
| Partition | hub를 나눈 병렬 칸 | 2개 |
| Consumer Group | 소비자들의 독립 읽기 채널 | 기본 `$Default` |
| Offset | 소비자가 "어디까지 읽었나" 표시 | SDP가 checkpoint로 관리 |

> **주의**: **Cosmos는 이 생산자–브로커–소비자 체인에 없습니다.** 브로커가 아니라 **저장소(DB)** 입니다. 데이터 경로가 둘로 나뉩니다.
> - **경로 A(실시간 이벤트)**: 단말기[생산자] → Event Hub[브로커] → SDP[소비자]
> - **경로 B(현재 상태)**: 단말기 → Cosmos[저장소]에 직접 write, SDP가 Cosmos를 read(참조)
> 경로 A만 큐/브로커 용어를 쓰고, 경로 B의 Cosmos는 "현황판(DB)"입니다.

#### 자주 헷갈리는 지점 (Q&A)

- **Q. Cosmos는 최신 데이터만 존재?** → 아니요. **`id=device_id` + `upsert`(덮어쓰기) 설계** 때문에 최신만 남는 것입니다. Cosmos 자체 성질이 아님. 이력을 원하면 id를 유니크하게 넣거나 Change Feed를 씁니다.
- **Q. Event Hub는 메모리 기반이라 사라지나?** → 아니요. **디스크에 복제 저장(내구성)** 됩니다. 사라지는 건 **보관기간(retention, Standard 1~7일)이 지나 "만료·삭제"** 되는 정책 때문. 휘발이 아니라 만료입니다. 그 전에 소비자가 읽어 Delta에 영구 저장하므로 무방.
- **Q. 그럼 Event Hub는 큐인가?** → **역할은 큐 계열(생산자↔소비자 완충)이 맞고**, 내부는 **"로그(log)"** 입니다. 전통 큐는 *읽으면 삭제*·경쟁 소비, 로그는 *읽어도 남고*·시간으로만 만료·여러 소비자 재생(replay) 가능. → Event Hub는 "메시지 큐"보단 **"이벤트 스트리밍 플랫폼(분산 로그)"**.
- **Q. 중개자와 브로커는 다른 것?** → 같습니다. 중개자=일상어, 브로커=기술용어. 여기선 둘 다 **Event Hub 하나**.
...
이건 **참조/차원 데이터**에 적합한 표준 패턴입니다. (상태 변화 자체를 실시간으로 받아야 하면 → Part 7 확장과제의 Change Feed로 승격)
...
## Part 5. 심화: DQX 데이터 품질 프레임워크 (SDP 보완)

SDP의 `expectations`만으로는 데이터 품질 관리가 다 커버되지 않습니다. 그래서 Databricks Labs의 **DQX** 프레임워크를 함께 씁니다. 이 프로토타입에서도 **실제로 우리 데이터에 DQX를 실행**해 차이를 확인했습니다.

### 5-1. 왜 SDP expectations만으로 부족한가

| 관점 | SDP expectations | DQX (Databricks Labs) |
|---|---|---|
| 실행 위치 | SDP 파이프라인 **안**에서만 | 파이프라인 **밖**에서도 (이미 적재된 데이터 사후검증 = at-rest) |
| 위반 진단 | 집계 지표(passed/failed 카운트) | **행/열 단위 상세** — `_errors`/`_warnings`에 규칙명·메시지·컬럼·함수 기록 |
| 심각도 | drop / fail / allow | **error(격리) vs warn(유지+표시)** 명확 분리 |
| 데이터셋 체크 | 행 단위 SQL 위주(어려움) | **is_unique · foreign_key(참조무결성) · aggregate · outlier · anomaly(ML)** |
| 규칙 정의 | 데코레이터/코드 | 코드(DQRule) + **YAML/config + no-code Studio UI** |
| 규칙 생성 | 수동 | **프로파일러 자동 생성**(SDP expectation 후보까지 생성) |
| 재사용 | 파이프라인 종속 | 여러 파이프라인/테이블에 재사용 |

> **한 줄**: SDP expectations = "파이프라인 흐름 중 1차 게이트", DQX = 그 위에 얹는 **정밀 품질 렌즈**(상세진단·심각도·데이터셋 체크·독립 검증).

### 5-2. 핵심 개념

- **DQEngine**: DQX 진입점 — `DQEngine(WorkspaceClient())`.
- **체크 정의**: YAML/dict(메타데이터) 또는 코드(`DQRowRule`/`DQDatasetRule`). 각 체크 = `criticality`(error|warn) + `check`(function + arguments).
- **적용**: `apply_checks_by_metadata(df, checks)` → 모든 행에 `_errors`/`_warnings` 부착(annotate). `..._and_split` → (valid, quarantine) 분리(단, split의 valid는 결과컬럼을 떨굼 → warn 보존하려면 annotate 사용).
- **결과 컬럼**: `_errors`/`_warnings` = 이슈 배열. 각 이슈에 `name·message·columns·filter·function·run_time`.

### 5-3. 우리 데이터에 적용한 체크 (코드)

원천: 이미 적재된 `bronze_telemetry`(600건)를 **SDP 밖에서** 파싱 → DQX 적용. (코드: [`realtime_source_proto/08_dqx_quality.py`](realtime_source_proto/08_dqx_quality.py))
...
- criticality: error         # 행 단위 → 위반 시 격리
  name: device_id_not_null
  check: {function: is_not_null, arguments: {column: device_id}}
- criticality: error
  name: value_in_metric_range
  check: {function: sql_expression, arguments: {expression: "...metric별 물리범위...", msg: value out of range}}
- criticality: warn          # 유지하되 _warnings 로 표시 (SDP drop 과 대비)
...
- criticality: error         # 데이터셋 체크 — SDP expectations로는 어려움
  name: seq_unique
  check: {function: is_unique, arguments: {columns: [seq]}}
- criticality: error         # 참조무결성: device_id 가 Cosmos 참조에 존재?
  name: device_in_reference
  check: {function: foreign_key, arguments: {columns: [device_id], ref_columns: [device_id],
          ref_table: adb_wrkspc_krc_dev.iot_proto.device_reference}}
""")

dq = DQEngine(WorkspaceClient())
annotated = dq.apply_checks_by_metadata(df, checks)   # 모든 행 + _errors/_warnings
```
> 실행: [`run_dqx.py`](realtime_source_proto/run_dqx.py) 가 노트북을 **serverless job**으로 제출합니다(`databricks-labs-dqx`는 PyPI 패키지 → serverless 환경 의존성으로 설치).

### 5-4. 실측 결과 — 같은 600건, 다른 처리

```
                       원천 600건
  SDP expectations →  silver 563  | 격리 37   (null 12 + 범위 14 + battery 11  전부 drop)
  DQX             →  valid  574  | quarantine 26 (error만: null 12 + 범위 14)
                                   + warn 103 (battery 11 + rssi 92) → valid에 "남기고 표시"
```

| 규칙 | 심각도 | 위반 | 결과 |
|---|---|---|---|
| device_id_not_null | error | 12 | quarantine |
| value_in_metric_range | error | 14 | quarantine |
| battery_healthy | **warn** | 11 | **valid 유지** + `_warnings` |
| signal_ok (rssi) | **warn** | 92 | **valid 유지** + `_warnings` |
| seq_unique / foreign_key / rowcount | error | 0 | 통과(참조무결성 정상) |

**핵심 대비**: SDP는 battery 불량 11건을 **drop**했지만, DQX는 이를 **warn**으로 두어 `valid`에 살리고 표시합니다. "버릴 정도는 아니지만 봐둬야 할" 데이터를 나누는 것 — SDP expectations로는 어려운 지점입니다.

**행별 상세 진단(핵심 차별점)** — 어떤 행이 **어느 규칙 때문에·왜** 걸렸는지 기록:
```
dev=SN-0009 metric=vibration val=292.65 → [value_in_metric_range] value out of physical range for metric
dev=(null)  metric=co2       val=502.12 → [device_id_not_null]     Column 'device_id' value is null
```
...
SELECT count(*) FROM dqx_annotated;    -- 600 (전체 + 진단)
SELECT count(*) FROM dqx_valid;        -- 574
SELECT count(*) FROM dqx_quarantine;   -- 26

-- error 규칙별 위반 (quarantine)
SELECT issue.name, issue.function, count(*) n
FROM (SELECT explode(_errors) issue FROM dqx_quarantine) GROUP BY 1,2 ORDER BY n DESC;

-- warn 규칙별 (valid에 보존된 경고 — SDP였다면 drop됐을 것)
SELECT issue.name, issue.function, count(*) n
FROM (SELECT explode(_warnings) issue FROM dqx_valid) GROUP BY 1,2 ORDER BY n DESC;

-- 행별 상세 진단 메시지
SELECT device_id, metric, value, issue.name, issue.message
FROM (SELECT device_id, metric, value, explode(_errors) issue FROM dqx_quarantine) LIMIT 5;
```

### 5-6. 언제 무엇을 쓰나 (실무 지침)

- **SDP expectations**: 파이프라인 흐름 안 **1차 방어**(빠른 drop/fail/격리), 스트리밍 인라인.
- **DQX**: ①이미 적재된 데이터 **사후 감사(at-rest)**, ②**상세 진단·리포트**, ③**데이터셋 체크**(유일성·참조무결성·이상탐지), ④**warn/error 세분화**, ⑤규칙을 **YAML/UI로 관리·재사용**.
- **함께**: SDP로 흐름 중 거르고, DQX로 at-rest 정밀 검증·모니터링. DQX 프로파일러로 **SDP expectation 후보를 자동 생성**해 둘을 잇기도 합니다.

> **한 줄 정리**: 같은 600건도 DQX는 **26 격리 + 574 유지(103 경고표시)** 로 처리해, "버릴 것"과 "봐둘 것"을 나눈다. SDP=1차 게이트, DQX=정밀 품질 렌즈.

---

## Part 6. 겪은 오류에서 배우기 (트러블슈팅)
...
| DQX `_warnings` 컬럼 못 찾음 | `apply_checks_..._and_split`의 valid는 결과컬럼을 **떨굼** | `apply_checks_by_metadata`(annotate)로 바꿔 모든 행에 진단 보존 | split은 "깨끗한 valid"용, warn 보존엔 annotate |
| DQX serverless 실행 | PyPI 패키지라 노트북에 필요 | job의 **serverless environment 의존성**(`databricks-labs-dqx`)으로 설치 | serverless에 라이브러리 올리는 법 |
...
| DQX | `08_dqx_quality.py` → `run_dqx.py` | DQX 데이터 품질검증(serverless job) |
